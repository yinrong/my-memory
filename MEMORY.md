...
- [[ai-hub-remote-server]] - ai-hub 腾讯云服务器 (111.229.143.69)，X+server 在远端，C 在本地
...
**主 Agent 循环**：
```
1. 读 FEATURES.md，找所有 🔲/⚠️ 条目 → 全部 ✅ 则结束
2. 按依赖分组，确定当前可并行的一批
3. 启动子 Agent 前：把对应条目标为 🔄 开发中
4. 并行启动若干子 Agent（每个传入：功能编号 + design doc 路径 + 代码库状态）
5. 等待子 Agent 汇报 PASS 后：
   - 过滤所有 🔄 条目，逐一检查是否完整（说明、测试列、Roadmap、README）
   - 确认无遗漏后改为 ✅
   - 检查是否解锁新条目
   - 卡住 → 介入分析，重新启动
6. goto 1
...
**🔄 开发中的价值**：完成时必须过滤所有 🔄 做全文一致性检查，避免只做点更新漏掉其他地方。

**FEATURES.md Roadmap 规范**：
- Roadmap"待完成"只列功能编号（如 AU1~AU4），不重复写说明
- 功能完成后立即从 Roadmap"待完成"中删除，只保留未完成项
- 状态的唯一真相是功能表的状态列，Roadmap 不是第二个状态源
...
- **用户消息以"需求:"开头** → 第一步必须在 FEATURES.md 新增/更新条目（🔲），再写 E2E 测试（FAIL），再实现（PASS），最后标 ✅。不允许先查代码或运行命令。
- **用户消息以"BUG:"开头** → 第一步必须在 FEATURES.md 已知 Bug 表新增条目（❌），再写 E2E 回归测试（FAIL），再修复（PASS），最后标 ✅ 并划掉 Bug 条目。不允许先查代码或运行命令。
...
- **✅ = E2E 测试跑通**，不是"代码写完"或"后端实现了"
  - 有 UI 的功能，前端未做 = ⚠️，不是 ✅
  - 子 Agent 汇报测试全绿时，主 Agent 必须核查：一个真实用户现在能用这个功能吗？
  - **不存在"待真机验证"状态** — 🔄 就是"代码写了但 E2E 未跑通"，跑通即改 ✅

### 环境操作规则（铁律，绝对不能违反）

**prod / prev 环境：零自主操作**
- **任何 pm2/进程操作（start/stop/restart/delete/reload/kill）一律禁止，除非用户在当前消息中明确说出"部署到 prev"或"部署到 prod"**
- 包括但不限于：重启、关闭、部署、更新、回滚、删除进程
- "代码改了需要生效"不是理由；"测试需要"不是理由；"顺手"不是理由
- 违反此规则 = 强制中断用户正在进行的工作，性质等同于数据丢失

**只有 dev / test 环境可自行操作**（重启、重建、删除均可，无需询问）

**判断标准**：操作前问自己：用户在这条消息里有没有说"部署到 prev"或"部署到 prod"？没有 → 绝对不碰。
...
| `README.md` | 人 | 是什么（**必须含用户视角的功能列表**，不是开发状态）、怎么装、怎么跑 |
...
**README 同步规则**：
- FEATURES.md 是开发追踪（状态/测试），**不能替代** README 的产品介绍
- 每当功能从 🔲/🔄 变为 ✅，必须检查 README 是否同步了该功能的用户可见描述
- 检查标准：一个陌生人看 README，能否知道这个项目能做什么？不能 → README 未完成


### 常见失败模式
- ❌ 认为自己"做不到"而要求用户手动操作
- ✅ 应该：检查已有工具，利用已知配置，自动完成
- ❌ 未经确认对 prod/prev 执行任何 pm2 操作（restart/stop/delete/reload/kill）
- ✅ 应该：只操作 dev/test；prod/prev 必须等用户明确说"部署到 prev/prod"
